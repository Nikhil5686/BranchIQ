...
const Alert = ({ msg, type = "success", onClose }) =>
  msg ? (
    <div
      className={`p-3 rounded-xl text-sm font-medium mb-4 flex justify-between items-center ${type === "success" ? "bg-green-50 text-green-700 border border-green-200" : "bg-red-50 text-red-700 border border-red-100"}`}
    >
      <span>{msg}</span>
...
function ProfilePanel() {
  ...
  // Auto-update form if user object changes (e.g. on fresh login/refresh)
  useEffect(() => {
    if (user) {
      setForm({
        name: user.name || "",
          ...
          >
            ...
            >
              <div className="flex justify-between items-center mb-2">
                <h3 className="text-xl font-extrabold text-gray-800">Update Profile Photo</h3>
                <button 
                  onClick={() => { stopCam(); setShowPhotoMode(false); }}
              ...
              <div className="flex gap-3">
                ...
                {camActive && (
                  <button
                    ...
                    disabled={loading}
                    className="flex-2 py-3 rounded-xl bg-[#185FA5] text-white font-bold text-sm shadow-md"
                  >
                    {loading ? "Uploading..." : "Capture & Use"}
                  </button>
                )}
              </div>
            </motion.div>
